...
import {
    getLastDayObj, getLastMonthObj,
    getLastWeekObj, getLastYearObj, getMonthday,
    getThisDayObj,
    getThisMonthObj,
    getThisWeekObj,
    getThisYearObj, getWeekday, months
} from "../util/date";
    ...
    if (period === 'year') {
        ...
        for (const month of months) {
            data.push({
                ...
                'New reservations': aw[month]?aw[month]['New reservations']:undefined,
                'Patients checkup': aw[month]?aw[month]['Patients checkup']:undefined
            });
        }
        return {
            data: data,
    ...
    if (period === 'month') {
        ...
        for (let i = 1; i <= 31; i++) {
            data.push({
                ...
                'New reservations': aw[i.toString()]?aw[i.toString()]['New reservations']:undefined,
                'Patients checkup': aw[i.toString()]?aw[i.toString()]['Patients checkup']:undefined
            });
        }
        return {
            data: data,
